...
# релятивистского электрического
# заряда в постоянном магнитном поле
...
# задание вектора безразмерной напряженности магнитного поля
B = np.array([0, 0, 1])


# задание функции, возвращающей значения
# координат вектора-функции,
# стоящей в левой части системы (6.57)
def Force(t, z):
    # инициализация массива F
    F = np.zeros(6)
    Coeff = np.sqrt(1 - (z[1] ** 2 + z[3] ** 2 + z[5] ** 2))
    F[0] = z[1]
    F[1] = 2 * np.pi * (B[2] * z[3] - B[1] * z[5]) * Coeff
    F[2] = z[3]
    F[3] = -2 * np.pi * (B[2] * z[1] - B[0] * z[5]) * Coeff
    F[4] = z[5]
    F[5] = 2 * np.pi * (z[1] * B[1] - z[3] * B[0]) ** Coeff
...
# на котором вычисляется численное решение СДУ (6.57)
Np = 1024
Tmax = 10
dt = Tmax / Np

# вычисление численного решения СДУ (6.57)
